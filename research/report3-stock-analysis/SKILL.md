...
**⚠️ execute_code Python 語法陷阱（實戰發現，2026-04-29）**

**千祈唔好用 inline Python f-string 嵌入 `{N/A condition}` 表達式**：

```python
# ❌ 會爆 NameError: name 'N' is not defined
f"""TK評分：{t("DFEN","total_score")}（{N/A if t("DFEN","total_score") is None else t("DFEN","total_score")}）"""

# ✅ 正確做法：喺 f-string 之前預先計算
dfen_score = t("DFEN","total_score")
dfen_score_str = str(dfen_score) if dfen_score is not None else "N/A"
f"""TK評分：{dfen_score_str}"""
```

原因：`N/A` 呢個三字符字符串喺 f-string 解析器入面會被當成未定義嘅 Python name，導致 `NameError`。正確做法係喺 f-string 之前用普通變量計算好再放進去。
...
```python
# TradingKey API — 直接用 urllib.request 喺 execute_code 最方便
import urllib.request, json

def fetch_tradingkey(symbol):
...
**⚠️ 唔好喺 execute_code 用 f-string 嵌入 Python code** — 字符 escaping 會出 SyntaxError。所有複雜 Python 用 write_file + terminal 執行。
...
**⚠️ 必須 `import os` 喺 script 入面** — 好容易漏，導致 NameError。

```python
# fetch_yf.py 內容
import yfinance as yf, json, pandas as pd, os
...
with open(os.path.expanduser("~/Investment/reports/.report3_data/yfinance.json"), "w") as f:
    json.dump(results, f)
```

### 2b. TradingKey SSR News — 同樣用 script file

TradingKey SSR news scraping 涉及複雜字符串處理（搵 JSON marker + brace matching），**必須用 write_file + terminal**，唔好嘗試喺 execute_code 入面 inline：

```python
# fetch_tk_news.py — 寫到 .report3_data/ 再 terminal 執行
import urllib.request, json, os
# ... (見 tradingkey skill 嘅 SSR scraping code)
```

### 3. Fragment Generation Workflow

**推薦用 execute_code 直接生成 + save fragments**（唔用 builder.py save_fragment）：

```
Step 1: terminal("python3 ~/Investment/reports/report3_builder.py clear")
Step 2: execute_code 生成 S1 + S2 fragments → write to .report3_fragments/
Step 3: execute_code 生成 S3 Chain fragments（每條 Chain 一個 fragment）
Step 4: execute_code 生成 S4 + S5 + S6 + S7 fragments
Step 5: execute_code merge all fragments → final MD（直接讀所有 .md files + concat）
Step 6: execute_code convert MD to HTML（簡化版 CSS OK，唔一定要用 template）
Step 7: send_message with MEDIA: HTML attachment
```

**⚠️ Fragment format mismatch（2026-04-27 實戰發現）**：`report3_builder.py` 內部 `list_fragments()` 係搜 `.html` files，但舊版 skill 話用 `.md` files。**兩個解決方案**：
1. 改用 builder.py 嘅 `save_fragment(section_id, html_content)` — 寫 HTML string 而唔係 .md（推薦，係 builder 原生介面）
2. 绕过 builder.py，直接喺 execute_code 讀所有 `.md` files + concat + 自己做 HTML generation（簡單場景 OK）

**推薦流程（唔再踩呢個坑）**：
```python
# execute_code 入面直接讀 .md files 做 merge
frag_dir = os.path.expanduser("~/Investment/reports/.report3_fragments/")
order = ["s1_context", "s2_cio", "s3_chain_AI", ...]
parts = []
for name in order:
    path = os.path.join(frag_dir, f"{name}.md")
    if os.path.exists(path):
        with open(path) as f:
            parts.append(f.read())
combined_md = "\n\n".join(parts)
# 然後自己做 md→HTML conversion 或用 template
```

**⚠️ Fragment merge 直接用 execute_code 讀所有 .md files + concat**（唔一定需要 builder.py merge 命令）。兩種方式都得，推薦用 execute_code 直接做，簡單清楚：
...
### 5. Execution Timing（實戰經驗）

**完整 Report 3 生成大約需要 8-12 分鐘**（cron timeout 15 分鐘）：
- Data fetching（TradingKey + yfinance + TK news）：3-4 分鐘
- Fragment generation（S1-S7）：3-4 分鐘
- Merge + HTML generation：1-2 分鐘
- Delivery：1 分鐘

**如果時間緊迫，可以 skip TradingKey SSR news**（最耗時嘅部分，每隻股票要 fetch + parse HTML）。用 WallStreetCN + yfinance news 替代。

### 6. Report 1/2 讀取注意

**Report 1/2 可能只有 HTML 版而冇 MD 版。** 讀取時要先試 .md，如果唔存在就 fallback 到 .html。HTML 版可以用 read_file 讀取（雖然內容有 HTML tags，但關鍵數據仍然可提取）。

## Delivery Troubleshooting（實戰經驗）

**問題：HTML attachment 冇收到，但 send_message 返回成功點算？**

Cron job 完成後檢查清單：
```
1. 確認 HTML 檔案存在：ls -la ~/Investment/reports/daily_report3_stock_analysis.html
2. 確認 size > 0（正常應有 30KB+）
3. 手動補發：send_message with MEDIA: path
4. 發送成功但冇 attachment → 檢查係咪 TEXT 模式發送（應為 HTML）
```

**預防：每次 cron 完成後，主動確認三份報告都有送到 Discord：**
```python
# Report 1/2/3 HTML files 存在 + size > 0 先行
# 有任何一份 size = 0 或 missing，手動補發
```

**注意：`send_message` 返回 `message_id` 只係代表 text 到咗，唔代表 attachment 到。**
HTML attachment 係另一個 delivery path，兩者獨立。
