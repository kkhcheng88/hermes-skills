...
## Compression Model Rate Limiting (429 Errors)

When context compression fails with 429 rate limit errors, the compression
model (used for summarization) may be hitting rate limits independently of
the main model. Common with free-tier OpenRouter models.

**Symptoms:**
- `Session compressed N times — accuracy may degrade`
- `Failed to generate context summary: Error code: 429`
- Subagent timeouts (300s) during compression

**Config location:** `auxiliary.compression.*` in config.yaml
(migrated from `compression.summary_*` in config version 17).

**Fields:** `provider`, `model`, `base_url`, `api_key`

**Fix:** Switch compression to a provider without rate limits. Example:
```yaml
auxiliary:
  compression:
    provider: custom
    ...
    api_key: sk-...  # Must be set explicitly!
```

**Pitfalls:**
- `api_key` is NOT auto-populated from `model.api_key` — must copy manually
- Cheaper/faster models are fine for summarization (just context compression)
- Requires `/new` session restart to take effect
